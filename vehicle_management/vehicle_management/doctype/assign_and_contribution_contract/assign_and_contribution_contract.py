...
def validate_vehicle(self):
    if self.status == "New":
        get_vehicle_status = frappe.get_value(
            "Vehicle Details", self.vehicle, "vehicle_status"
        )
        if get_vehicle_status == "Not available":
            self.set("vehicle", "")
            frappe.throw(
                _(
                    "Cannot save, the vehicle is already assigned to another contract !!!"
                )
            )
...
def make_invoice(vehicle, amount, mode_of_payment, name=None, customer=None):
    ...
    if source:
        ...
        if not default_customer and not customer:
            frappe.throw(
                _("Please set default customer in the vehicle management Settings")
            )
        if customer:
        ...
        invoice.append(
            "items",
            dict(
                item_code=item, qty=1, rate=amount, cost_center=get_cost_center(vehicle)
            ),
        )
        # invoice.is_pos = 1
