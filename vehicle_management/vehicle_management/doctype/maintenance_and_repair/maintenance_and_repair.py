...
class Maintenanceandrepair(Document):
    def generate_schedule(self):
        self.set("schedules", [])
        frappe.db.sql(
            """delete from `tabMaintenance checks`
			where parent=%s""",
            (self.name),
        )
        count = 1
    ...
    def create_vehicle(self):
        if self.vehicle_contract:
            vehicle = frappe.get_value(
                "Assign and contribution contract", self.vehicle_contract, "vehicle"
            )
            if vehicle:
    ...
    def create_vehicle_contract(self):
        if self.vehicle:
            vehicle_contract = frappe.get_value(
                "Assign and contribution contract", self.vehicle, "name"
            )
            if vehicle_contract:
        ...
        if self.maintenance_check_button:
            ...
            for d in self.get("maintenance_checks"):
                if not d.item_code:
                    ...
                    throw(
                        _(
                            "Please select Start Date and End Date for Item {0}".format(
                                d.item_code
                            )
                        )
                    )
    ...
    def validate_dates(self):
        ...
        if last_inv:
            ...
            if start_date_inv <= start_date <= end_date_inv:
                frappe.msgprint(
                    _(
                        "The dates correspond from what is in the invoice, dates has been regenerated"
                    )
                )
        ...
        if source.company_pay:
            ...
            if not source.driver_employee and not source.driver_customer:
                ...
                if not default_customer:
                    frappe.throw(
                        _(
                            "Please set default customer in the vehicle management Settings"
                        )
                    )
            ...
            if not item:
                frappe.throw(
                    _("Please set default item in the vehicle management Settings")
                )
            ...
            invoice.submit()
            frappe.msgprint(
                _("Invoice created successfully for {0}".format(source.vehicle))
            )
            return invoice
...
def get_last_tc_invoice(name):
    ...
    doc = frappe.get_all(
        "Purchase Invoice",
        ["name"],
        [["docstatus", "!=", 2], ["is_return", "!=", 1], ["vehicle", "=", name]],
