...
def get_column():
    return [
        _("Vehicle") + ":Link/Vehicle Details:120",
...
def get_data(conditions, filters):
    vehicle_details = frappe.db.sql(
        """ select `tabAssign and contribution contract`.vehicle, `tabAssign and contribution contract`.customer, `tabAssign and contribution contract`.customer_name,
		...
		`tabAssign and contribution contract`.vehicle = `tabVehicle Details`.vehicle_registration and %s order by `tabAssign and contribution contract`.vehicle"""
        % conditions,
        filters,
        as_list=1,
    )
